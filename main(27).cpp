...
vector<vector<int>> entradaPistes(int n) {
    ...
    for (int i = 0; i < n ; ++i) {
        //Inv: Tots els i - 1 blocs fins ara han estat inputats i han cambiat el tamany del vector pistes[i].
	int blocs;
        cin >> blocs; // Entrada de blocs
        pistes[i].resize(blocs); // Remodelació del vector posterior a l'entrada
        for (int j = 0; j < blocs ;++j) {
	    //Inv: els j - 1 elements inputats fins ara han estat guardats al vector pistes 'i' actual.
            cin >> pistes[i][j];
        }
    }
    return pistes;
...
//PRE: Es reb tres matrius: el tauler actual, les pistes de les files, i les pistes de las columnes
//POST: no es retorna dades (funció void). Igualment, s'ha imprés el tauler actual amb les pistes.
void mostrarTauler(const vector<vector<char>>& tauler, const vector<vector<int>>& pistesFiles, const vector<vector<int>>& pistesColumnes) {
    ...
    for (int j = 1; j <= columnes; ++j) {
	    	//Inv: totes les j - 1 columnes fins ara han estat enumerades al tauler.
		if(j < 11) {
			cout << " " << j << " ";
		} else {
			cout << j << " ";
    ...
    cout << "    ";
    for (int j = 1; j <= columnes; ++j) {
	//Inv: Els j - 1 espais fins ara han estat impressos i estàn en procès de formar la línea separadora superior del tauler.
        if(j==columnes){
            cout << "--";
        } else {
            cout << "---";
    ...
    for (int i = 0; i < files; ++i) {
    		//Inv: totes les i - 1 files de pistes fins ara han estat enumerades.
		if(i < 9) {
			cout << " ";
		}
        	cout << (i + 1) << " |"; // Numeración de files
        	for (int j = 0; j < columnes; ++j) {
			//Inv: tots els j - 1 espais han estat impresos. 
			cout << tauler[i][j];
            		if(j < columnes - 1) {
                	cout << "  ";
            			} else {
			...
			//Inv: tots els j - 1 elements de pistes fins ara han estat afegits.
    ...
    cout << "    ";
    for (int j = 1; j <= columnes; ++j) {
	//Inv: Els j - 1 espais fins ara han estat impressos i estàn en procès de formar la línea separadora inferior del tauler.
        if(j==columnes){
            cout << "--";
        } else {
            cout << "---";
    ...
    for (unsigned i = 0; i < pistesColumnes.size(); ++i) {
        //Inv: els i - 1 vectors abans del vector pistesColumnes[i] han estat accedits i tractats.
    ...
    for (int i = 0; i < maxpistesCol; ++i) {
        //Inv: s'ha accedït totes les files abans de l'índex i i s'han impres els espais correctament.
	cout << "   ";
        for (int j = 0; j < columnes; ++j) {
        	//Inv: s'ha accedït totes les files abans de l'índex i i s'han impres els espais correctament.
...
int main() {
	...
	while(continuar){
		//Inv: continuar es true en aquest moment.
		cin >> opc;
        ...
        // Marcar conjunt de caselles - OPCIÓ B
			...
			for(int i = fila1 - 1; i < fila2; ++i){
				//Inv: s'ha accedït a les i - 1 files.
				for(int j = columna1 - 1; j < columna2; ++j){
					//Inv: s'ha accedït als j - 1 elements dins del vector tauler[i].
					tauler[i][j] = 'X';
				}
			}
		}   
